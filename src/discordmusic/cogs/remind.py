...
class RemindBot(commands.Cog):
    ...
    async def remind_task(self):
        ...
        for reminder in self.reminders:
            ...
            if abs((now - when).total_seconds()) < 1.0:
    ...
    async def _remind(
        self, interaction: discord.Interaction, what: str, when: str, who: discord.User = None
    ):
        ...
        await interaction.response.send_message(
            f'<@{who_id}> will be reminded at {when.strftime("%I:%M:%S %p")} on {when.strftime("%m-%d-%Y")}.',
            ephemeral=True,
        )

    @app_commands.command(
        name='sleep', description='Set a sleep timer to disconnect from a voice channel.'
    )
    async def _sleep(self, interaction: discord.Interaction, when: str):
        when = timezone('EST').localize(parse(when))
        self.sleep_timers.append((interaction.user.id, when))
        await interaction.response.send_message(
            f'<@{interaction.user.id}> will be disconnected at {when.strftime("%I:%M:%S %p")} on {when.strftime("%m-%d-%Y")}.',
            ephemeral=True,
        )
