...
class VoiceBot(commands.Cog):
    ...
    async def _channel(
        ...
        if channel.id in self.channels:
            ...
            await interaction.response.send_message(
                f'Updated Settings for <#{channel.id}>.', delete_after=5
            )
        else:
            if status and len(status) > 0:
                await channel.edit(status=status)
                ...
                await interaction.response.send_message(
                    f'Permission Denied for <#{channel.id}>.', delete_after=5
                )
    ...
    async def create_channel(self, member, category):
        ...
        channel = await category.create_voice_channel(
            f"{member.nick}'s Channel #{count}", bitrate=self.bitrate
        )
    ...
    async def on_voice_state_update(self, member, before, after):
        if before.channel:
            if (
                before.channel.id in self.channels
                and len(self.bot.get_channel(before.channel.id).members) == 0
            ):
