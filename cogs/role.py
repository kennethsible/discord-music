...
import webcolors, discord, json, re
...
class RoleBot(commands.Cog):
    ...
    async def _role(self, interaction: discord.Interaction, who: discord.User, name: str, color: str = '#ffffff'):
        if not re.match(r'#[A-Fa-f0-9]{6}', color):
            try:
                color = webcolors.name_to_hex(color.replace(' ', '').lower()).lstrip('#')
            except ValueError:
                raise app_commands.AppCommandError(f'\'{color}\' is not a valid css3 or hexadecimal color.')
        if who.id in self.roles:
            ...
            role = await interaction.guild.create_role(name=name,
                color=discord.Color(int('0x' + color, 16)))
            self.roles[who.id] = role.id
            await who.add_roles(role)
        await interaction.response.send_message(f'Created Role <@&{role.id}> for <@!{who.id}>.',
